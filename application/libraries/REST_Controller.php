...
abstract class REST_Controller extends CI_Controller
{
	...
	public function __construct()
	{
		...
		// #CUSTOM ---------------------------------------------------------
		// LOAD DATABASE MODEL
		$this->load->model('database_scheme_model');
	...
	public function _remap($object_called, $arguments)
	{
		...
		if (config_item('rest_enable_keys') AND $use_key AND $this->_allow === FALSE)
		{
			...
			// #CUSTOM -----------------------------------------------------------------
			$this->response(array('status' => false, 'error' => 'Usuário/Senha inválido'), 403);
		}
		...
		if($this->_permission !== TRUE){
			$this->response($this->_permission, $this->_permission['error_code']);
			//$this->response(array('status' => false, 'error' => 'Seu usuário não tem permissão de acesso ao Dw Força de Vendas. Contate o administrador do sistema.', 'type' => 'permission'), 401);
		}
	...
	protected function _detect_api_key()
	{
		...
		$auth = config_item('sync_table_auth');
		...
		{
			$hashPswd = @$_POST['hashPswd'] ? $_POST['hashPswd'] : $key;
			if($hashPswd == $auth['apiKeyMaster'])
			{
				return TRUE;
			}
			if ( ! ($row = $this->rest->db->where(config_item('rest_key_field'), $key)->get(config_item('rest_keys_table'))->row()))
			{
				return FALSE;
			}
	...
	// 
	/**
	 * Verificia se o usuario está ativo e possui permissão de acesso no Força de Vendas
	 * @author Willian H. M.
	...
	{	
		...
		$tableAuth = $this->config->item('sync_table_auth');
		$headers = $this->input->request_headers();

		$user = $this->database_scheme_model->getUser($headers['Api-key'], $headers['Login']);

		if($this->input->post() && $this->input->post('lockedApplication') == true){
			return TRUE;
		}

		if($user){
			$this->user = $user;
			if(!$user['permissao_produto'] || !$user['ativado']){
				return array('status' => false, 'error' => 'Seu usuário não tem permissão de acesso ao Dw Força de Vendas. Contate o administrador do sistema.', 'type' => 'permission', "error_code" => 401);
			}
			
			if(count($user['vendedores_codigo']) == 0){
				return array('status' => false, 'error' => 'Seu usuário não possui um vendedor vinculado. Contate o administrador do sistema.', 'type' => 'vendedor_error', "error_code" => 400);
			}
